...
public class SimulateCombat {
	...
	private static void initEntities() {
		...
		final Item weapon = em.getItem("maczuga");
		final Item weapon_5 = em.getItem("sztylet dusz");
		final Item shield = em.getItem("drewniana tarcza");
		final Item armor = em.getItem("koszula");
		final Item helmet = em.getItem("skórzany hełm");
		final Item legs = em.getItem("skórzane spodnie");
		final Item boots = em.getItem("buty skórzane");

		if (creature_name == null) {
			enemy = new Creature("dummy", "dummy", "(generic creature)", hp, atk, atk, def, lvl,
				1, 1, 1, 1.0, 100, 100, new ArrayList<>(), new HashMap<>(), new LinkedHashMap<>(), 1, "dummy");
		} else {
			if (creatures == null) {
