...
public class SoundSystemNG extends Thread {
	private static class SystemOutput {
		...
		boolean isReady() {
			return mLine.isOpen() && mLine.isRunning();
		}
...
//		int     getNumSamples       () { return mNumSamples;                               }
		...
		int getNumBytesPerSample() {
			return mFormat.getSampleSizeInBits() / 8;
		}

		int available() {
			return mLine.available();
		}

		int getNumBytesToWrite() {
			return mNumBytesToWrite;
		}

		int getFrameSize() {
			return getNumBytesPerSample() * getNumChannels();
		}
	...
	public static class Output extends SignalProcessor {
		...
		public void setIntensity(float intensity) {
			mIntensity.set(Numeric.floatToInt(intensity, PRECISION));
		}

		public float getIntensity() {
			return Numeric.intToFloat(mIntensity.get(), PRECISION);
		}
		...
		int getNumChannels() {
			return mAudioFormat.getChannels();
		}

		int getSampleRate() {
			return (int) mAudioFormat.getSampleRate();
		}
		...
		boolean mix(float[] buffer, int size) {
			...
			while (numSamplesToMix > 0) {
				...
				Dsp.mixAudioData(buffer, offset, mAudioBuffer, mNumSamplesMixed, numSamples,
						Numeric.intToFloat(mIntensity.get(), PRECISION));
	...
	private void threadSleep(long nanos) {
		...
		try {
			...
			// Guard against "nanosecond timeout value out of range" when stopping the sound system.
