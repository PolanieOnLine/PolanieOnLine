...
class StatusIconPanel extends JComponent {
	...
	private static final ImageIcon chokingIcon = new ImageIcon(
			DataLoader.getResource("data/sprites/ideas/choking.png"));
	 ...
	 *
	 * @param ID      The ID value of the status
	 * @param visible Show the icon
	 */
	...
	void setAway(String message) {
		...
		if (isAway) {
			if ((awayMessage == null) || !message.equals(awayMessage)) {
				...
				if (away.getAccessibleContext() != null) {
	...
	void setGrumpy(String message) {
		...
		if (isGrumpy) {
			if ((grumpyMessage == null) || !message.equals(grumpyMessage)) {
				...
				if (grumpy.getAccessibleContext() != null) {
