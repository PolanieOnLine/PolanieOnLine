 ...
 *                                                                         *
 ***************************************************************************/
package games.stendhal.server.maps.gdansk.bank;
...
public class TellerNPC implements ZoneConfigurator {
	...
	private SpeakerNPC buildNPC() {
		final SpeakerNPC npc = new SpeakerNPC("Magdalena");

		npc.setOutfit("body=1,head=0,eyes=1,hair=3,dress=53");
		npc.setOutfitColor("hair", 0x691313);
		npc.setGender("F");
		npc.setPosition(20, 3);
		npc.setIdleDirection(Direction.DOWN);

		npc.addGreeting("Witamy w banku w przybrzeżnym mieście!");
		npc.addGoodbye();
