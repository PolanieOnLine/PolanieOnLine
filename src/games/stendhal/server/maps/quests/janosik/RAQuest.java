...
public class RAQuest implements IRAQuest {
	...
	public RAQuest(Map<String, Integer> timings) {
	...
	public void phaseToDefaultPhase(List<String> comments) {
		...
		Janosik.setPhase(Janosik.getDefaultPhaseClass().getPhase());
		logger.info("Janosik quest: switch phase ("+
				Janosik.getPhase().name()+
				") to ("+
				Janosik.getDefaultPhaseClass().getPhase().name());
		stopShouts();
		Janosik.setNewNotificationTime(
				Janosik.getDefaultPhaseClass().getMinTimeOut(),
				Janosik.getDefaultPhaseClass().getMaxTimeOut());
		Janosik.getDefaultPhaseClass().prepare();
		if(!comments.isEmpty()) {
	...
	public void phaseToNextPhase(IRAQuest nextPhase, List<String> comments) {
		...
		logger.info("Janosik quest: switch phase to ("+nextPhase.getPhase().name()+").");
		Janosik.setPhase(nextPhase.getPhase());
		stopShouts();
		Janosik.setNewNotificationTime(
