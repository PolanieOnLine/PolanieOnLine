...
public class InspectQuestAction extends AdministrationAction {
	...
	public static void register() {
		CommandCenter.register(INSPECTQUEST, new InspectQuestAction(), 6);
	}
	...
	protected void perform(final Player admin, final RPAction action) {
		...
		if (target == null) {
			final String text = "Nie odznaleziono jednostki dla akcji:" + action;
		...
		if (target instanceof Player) {
			...
			if (action.has("quest_slot")) {
				...
				// get all quests
				final StringBuilder sb = new StringBuilder("Stany zadań dla gracza " + player.getName() + ":");
