...
public class StendhalItemDAO {
	...
	private void dump(PreparedStatement stmt, DefaultItem item) throws SQLException {
		...
		stmt.setString(8, item.getAttributes().get("min_use"));
		stmt.setString(9, item.getAttributes().get("min_level"));

		stmt.setString(10, item.getAttributes().get("atk"));
		stmt.setString(11, item.getAttributes().get("ratk"));
		stmt.setString(12, item.getAttributes().get("rate"));
		stmt.setString(13, item.getAttributes().get("def"));
		stmt.setString(14, item.getAttributes().get("range"));
		stmt.setString(15, toStringOrNull(item.getDamageType()));
		stmt.setString(16, item.getAttributes().get("lifesteal"));

		stmt.setString(17, item.getAttributes().get("amount"));
		stmt.setString(18, item.getAttributes().get("regen"));
		stmt.setString(19, item.getAttributes().get("frequency"));
		stmt.setString(20, item.getAttributes().get("immunization"));
		stmt.setString(21, item.getAttributes().get("antipoison"));
		stmt.setString(22, item.getAttributes().get("life_support"));

		stmt.setString(23, toStringOrNull(item.getImplementation()));
		...
		stmt.setString(29, item.getAttributes().get("slot_name"));
		stmt.setString(30, item.getAttributes().get("slot_size"));
		
		stmt.setString(31, item.getAttributes().get("undroppableondeath"));
		stmt.setInt(32, MathHelper.parseIntDefault(item.getAttributes().get("autobind"), 0));
		stmt.setString(33, item.getAttributes().get("max_quantity"));
		stmt.setString(34, item.getAttributes().get("deterioration"));
		stmt.setInt(35, item.isUnattainable() ? 1: 0);
	...
	public void dump(DBTransaction transaction) throws SQLException {
		...
		for (DefaultItem item : defaultItems) {
			...
			stmt.setString(36, item.getItemName());
			dump(stmt, item);
		}
		stmt.executeBatch();
