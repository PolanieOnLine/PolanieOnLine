...
public class OwnedItem extends Item {
	...
	public String describe() {
		...
		if (owner != null) {
			description += " To " + getTitle() + " należy do " + owner + " oraz nie może zostać użyte przez innych.";
		}
	...
	public void onEquipFail(final RPEntity entity, final String slot) {
		if (entity instanceof Player) {
			...
			if (ownedBlacklistSlots.contains(slot)) {
				player.sendPrivateText("Nie możesz mieć ze sobą tej własności " + getTitle() + ".");
			} else if (ownerOnlySlots.contains(slot)) {
				player.sendPrivateText("Tylko " + getOwner() + " może nosić to " + getTitle() + ".");
			}
		}
	}
