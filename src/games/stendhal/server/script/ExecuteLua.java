...
public class ExecuteLua extends ScriptImpl {
	...
	public void execute(final Player admin, final List<String> args) {
		if (args.isEmpty()) {
			admin.sendPrivateText(NotificationType.ERROR, "Proszę sprecyzować skrypt, by załadować: /script ExecuteLua.class script_path");
			return;
		}

		final File script = Paths.get("data/script", String.join(" ", args)).toFile();
		if (!script.isFile()) {
			admin.sendPrivateText(NotificationType.ERROR, "Wystąpił błąd podczas próby załadowania skryptu Lua. Nie odnaleziono pliku: " + script.toString().replace("\\", "\\\\"));
			return;
		}
