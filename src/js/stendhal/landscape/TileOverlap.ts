...
export function getTileOverlapMetrics(tileScale: number, baseEdgeTrim = BASE_TILE_EDGE_TRIM, pixelRatio = 1, tileSize = 32): TileOverlapMetrics {
	...
	const needsOverlap = clampedScale <= 1;
	const alignmentPadding = clampedScale < 1 && Math.abs(scaledTilePixels - Math.round(scaledTilePixels)) > 0.001
		? 1
		: 0;
	const minOverlap = needsOverlap ? Math.max(1, Math.ceil(2 / Math.max(1, effectivePixelRatio))) : 0;
	// A small overlap is also required at 1:1 scale on mobile browsers. Without
	// it Chromium can expose one-pixel seams between independently drawn tiles.
	const tileOverlap = needsOverlap ? Math.max(minOverlap, Math.ceil(2 / clampedScale)) + alignmentPadding : 0;
	const overlapOffset = tileOverlap ? Math.floor(tileOverlap / 2) : 0;
	const edgeTrim = needsOverlap ? Math.max(baseEdgeTrim, baseEdgeTrim / clampedScale) : baseEdgeTrim;
